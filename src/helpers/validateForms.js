...
export const SignupSchemaEN= Yup.object().shape({
  ...
  export  const SignupSchemaES = Yup.object().shape({
    name: Yup.string().required('El nombre es requerido'),
    phoneNumber: Yup.string().matches(/^\d{10}$/, 'El número de teléfono debe tener 10 dígitos').required('El número de teléfono es requerido'),
    password: Yup.string().matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/, "La contraseña debe tener más de 8 caracteres entre mayúsculas, minúsculas y números").required('La contraseña es requerida'),
    ...
    corporateEmail: Yup.string().email('Correo electrónico corporativo inválido').matches(/^(?!.*@(?:hotmail\.com|gmail\.com|yahoo\.com |outlook\.com)$)([\w.%+-]+@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,})$/, 'El correo electrónico debe ser de la empresa').required('El correo electrónico corporativo es requerido'),
  });
